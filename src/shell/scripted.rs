...
use super::{CommandResult, Shell, ShellConfig, ShellError};
...
mod tests {
    ...
    fn scripted_shell_success_result() {
        ...
        let result = shell
            .run_command("build", "unused", &[], &mut out, default_mode())
            .unwrap();
    ...
    fn scripted_shell_failure_result() {
        let shell =
            ScriptedShell::new().push(Script::new().err_line("something broke").exit_failure());
        let mut out = StringOutput::new();
        let result = shell
            .run_command("deploy", "unused", &[], &mut out, default_mode())
            .unwrap();
        assert!(!result.success);
    }
    ...
    fn scripted_shell_stderr_captured_in_result() {
        let shell =
            ScriptedShell::new().push(Script::new().err_line("warn: low disk").exit_failure());
        let mut out = StringOutput::new();
        let result = shell
            .run_command("check", "unused", &[], &mut out, default_mode())
            .unwrap();
    ...
    fn scripted_shell_multiple_stderr_lines_joined() {
        let shell = ScriptedShell::new().push(
            Script::new()
                .err_line("error: line 1")
                .err_line("error: line 2")
                .exit_failure(),
        );
        let mut out = StringOutput::new();
        let result = shell
            .run_command("test", "unused", &[], &mut out, default_mode())
            .unwrap();
    ...
    fn scripted_shell_step_result_written_to_output() {
        ...
        let mut out = StringOutput::new();
        shell
            .run_command("mytask", "unused", &[], &mut out, default_mode())
            .unwrap();
    ...
    fn scripted_shell_multiple_scripts_consumed_in_order() {
        ...
        let r1 = shell
            .run_command("step1", "unused", &[], &mut out, default_mode())
            .unwrap();
        let r2 = shell
            .run_command("step2", "unused", &[], &mut out, default_mode())
            .unwrap();
    ...
    fn scripted_shell_empty_queue_panics() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            shell
                .run_command("oops", "unused", &[], &mut out, default_mode())
                .unwrap();
        }));
    ...
    fn scripted_shell_shell_exec_returns_success() {
        ...
        let mut out = StringOutput::new();
        let result = shell
            .shell_exec("echo hi", &mut out, default_mode())
            .unwrap();
        assert!(result.success);
    }
    ...
    fn scripted_shell_exec_capture_returns_success() {
        ...
        let mut out = StringOutput::new();
        let result = shell
            .exec_capture("echo hi", &mut out, default_mode())
            .unwrap();
        assert!(result.success);
    }
    ...
    fn scripted_shell_exec_interactive_returns_ok() {
        ...
        assert!(
            shell
                .exec_interactive("echo hi", &mut out, default_mode())
                .is_ok()
        );
    }
    ...
    fn scripted_shell_with_config_accepts_custom_viewport() {
        ...
        let shell = ScriptedShell::new().with_config(config).push(
            Script::new()
                .out_line("line 1")
                .out_line("line 2")
                .out_line("line 3"),
        ...
        let result = shell
            .run_command("task", "unused", &[], &mut out, default_mode())
            .unwrap();
        assert!(result.success);
    }
}
